...
CONFIG_INPUT="/usr/share/X11/xorg.conf.d/40-libinput.conf"
...
if [ ${STATUS} -eq 0 ]; then
    ...
    if [ -e "${CONFIG_INPUT}" ]; then
        grep -e "^\        ${CONFIG_OPTION}${CONFIG_LINE}" ${CONFIG_INPUT} > /dev/null
        STATUS=$?
        if [ $STATUS -eq 1 ]; then
            sudo sed -i "/${CONFIG_OPTION}/d" ${CONFIG_INPUT}
            sudo sed -i "/${INPUT_CLASS}/a\        ${CONFIG_OPTION}${CONFIG_LINE}" ${CONFIG_INPUT}
            ks_restart=1
        fi
    fi
else
...
if [ ${rm_display} -eq 1 ]; then
    if [ -e "${DISPLAY_CONFIG}" ]; then
        sudo rm ${DISPLAY_CONFIG}
        ks_restart=1
    fi
fi
if [ ${rm_input} -eq 1 ]; then
    if [ -e "${CONFIG_INPUT}" ]; then
        sudo sed -i "/${CONFIG_OPTION}/d" ${CONFIG_INPUT}
        ks_restart=1
    fi
